...
class Trade():
    ...
    def new_trade(self, trade_id: str, order_type: str, side: str, enter_or_exit: str, price:float = 0.0, stop_limit_price: float = 0.0) -> dict:
        ...
        self.order = {
            'orderStrateguType': 'SINGLE',
            'orderType': self.order_types[order_type],
            'session': 'NORMAL',
            'duration': 'DAY',
            'orderLegCollection': [
                {
                    'intruction': self.order_instructions[enter_or_exit][side],
                    'quntity': 0,
                    'instrument': {
                        'symbol': None,
                        'assetType': None
    ...
    def add_box_range(self, profit_size: float = 0.00, percentage: bool = False, stop_limit: bool = False):
        ...
        if not stop_limit:
            self.add_stop_los(stop_size=profit_size, percentage=percentage)
    ...
    def _calculate_new_price(self, price: float, adjustmnet: float, percentage: bool) -> float:

        if percentage:
            new_price = price * adjustmnet
        else:
            new_price = price + adjustmnet
