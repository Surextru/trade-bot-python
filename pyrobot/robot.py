...
class PyRobot():
    ...
    def _create_session(self) -> TDClient:
        """
        td_client es un objeto que contiene los datos que QUIEN es la persona (en plan email y pass)
        """
        td_client = TDClient(
    ...
    def regular_market_open(self) -> bool:
        ...
        pmarket_start_time = datetime.now().replace(hour = 13, minute = 30, second = 00, tzinfo = timezone.utc).timestamp()
        market_end_time = datetime.now().replace(hour = 20, minute = 00, second = 00, tzinfo = timezone.utc).timestamp()
        right_now = datetime.now().replace(tzinfo = timezone.utc).timestamp()

        if market_end_time >= right_now >= pmarket_start_time:
            return True
        else: 
            return False
